    ...
    <section id="experience" className="w-full pt-2 md:pt-4 pb-0">
      <div className="max-w-5xl mx-auto px-4 md:px-6">
        <div className="relative">
          {/* Vertical line */}
          <div className="absolute left-4 sm:left-6 top-0 bottom-0 border-l border-[#D4AF37]/30" />
          ...
          <ul className="space-y-8 md:space-y-10">
            {items.map((item, idx) => (
              <li key={idx} className="relative pl-12 sm:pl-16">
                {/* dot */}
                <span
                  className="absolute left-0 sm:left-2 top-2 h-3.5 w-3.5 rounded-full"
                ...
                <motion.div
                  ...
                  className="rounded-xl border border-[#D4AF37]/25 p-4 sm:p-5 bg-transparent hover:bg-white/5 transition-colors"
                >
                  <div className="flex flex-wrap items-baseline gap-2">
                    ...
                    {item.certLink ? (
                      <a
                        ...
                        className="inline-flex items-center gap-1 rounded-full border border-[#D4AF37]/40 px-3 py-1 text-[11px] text-[#FFD700]/90 hover:text-white hover:bg-[#D4AF37]/20 transition-colors mr-2"
                  ...
                  <div className="mt-3 grid md:grid-cols-5 gap-4 items-start">
                    <div className="md:col-span-2 flex md:block justify-between md:justify-start">
                      <div className="text-neutral-400 text-sm md:text-base">{item.start} — {item.end}</div>
                      {item.stack?.length ? (
                        <div className="mt-0 md:mt-3 flex flex-wrap gap-2">
                          {item.stack.map((s) => (
                            <span
                              key={s}
                              className="text-xs px-2 py-1 rounded border border-[#D4AF37]/30 text-[#FFD700]/80"
                            >
                              {s}
                            </span>
                          ))}
                    ...
                    <ul className="md:col-span-3 list-disc pl-5 text-neutral-300 space-y-1.5">
                      {item.bullets.map((b) => (
                        <li key={b}>{b}</li>
                      ))}
                    </ul>
